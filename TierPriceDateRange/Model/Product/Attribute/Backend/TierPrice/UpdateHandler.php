...
class UpdateHandler extends AbstractHandler
{
    ...
    protected function getAdditionalFields(array $objectArray): array
    {
        ...
        // adding date_from and date_to attributes to getAdditionalFields() function
        return array_merge(
            $result,
            [
    ...
    private function updateValues(array $valuesToUpdate, array $oldValues): bool
    {
        ...
        foreach ($valuesToUpdate as $key => $value) {
            if ((($value['value'])!== null
                ...
                || $this->getDateAttribute($oldValues[$key], 'date_from')
                !== $this->getDateAttribute($value, 'date_from')
                || $this->getDateAttribute($oldValues[$key], 'date_to')
                !== $this->getDateAttribute($value, 'date_to')
            ) {
                $price = new DataObject(
                    [
                        ...
                        'date_from' => $this->getDateAttribute($value, 'date_from'),
                        'date_to' => $this->getDateAttribute($value, 'date_to')
                    ]
                );
